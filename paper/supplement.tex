...
% Cross-document references into the main paper. Reads main.aux to
% resolve \ref{tab:...} that point at labels in the main paper.
% Requires main.pdf to have been built at least once (build.sh
% builds main first, then supplement, so the .aux is always present
% by the time bibtex/pdflatex run on supplement.tex).
\usepackage{xr}
...
\subsection{Multi-family solver matrix}
\label{sup:multi-family-solver}

The \S2.1 and \S2.2 tables aggregate per-cell relative-$L^2$
numerically across the ODE and PDE solver matrices. For readers who
want the visual cross-check, this subsection adds two figures: the
peer-review audit iteration that drove the audit-corrected
configurations, and the multi-family quantum-PINN matrix
visualization spanning four SOTA ansatz families on three PDEs from
the hardness ladder. Both figures source data directly from the
JSONs underpinning the tables above.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_p3_8_review_iteration.pdf}
\caption{Peer-review audit iteration on the solver hardness ladder. Row~0: quantum chebyshev\_dqc\_2d vs.\ a capacity-matched classical MLP-PINN baseline (log-scale relative-$L^2$) on heat, smooth Burgers, and Allen--Cahn at audit-corrected configurations (1200--1800 training steps; $64\times32$ collocation for Allen--Cahn). Row~1: per-system loss trajectories (seed 0) diagnosing convergence vs.\ saturation. Row~2: periodic-boundary-condition violation $\max_t \lVert u(\cdot,t) - u(\cdot,t)|_{\text{periodic}}\rVert / \max\lVert u\rVert$; both quantum and classical PINNs exceed the 5\% design target (relative violation $\approx 0.6$--$1.6$), confirming the audit prediction that the Lagaris initial-condition ansatz does not implicitly enforce periodicity. Row~3: four quantum families (chebyshev\_dqc, te\_qpinn\_fnn, te\_qpinn\_qnn, qcpinn) on Lorenz extended to $T=5$ ($\approx 5.5$ Lyapunov times) against a predict-mean baseline (replacing the predict-zero floor of P3.6). Source: \texttt{results/p3\_8\_review/}.}
\label{sup:fig:p3-8-audit-review}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_p3_9_pde_matrix.pdf}
\caption{Multi-family solver matrix: four SOTA quantum-PINN families (chebyshev\_dqc, qcpinn, TE-QPINN with FNN encoding, TE-QPINN with QNN encoding) compared against a classical MLP-PINN baseline on three PDEs from the hardness ladder (heat, smooth Burgers, Allen--Cahn; $n=3$ seeds per cell). Row~0: relative-$L^2$ accuracy with 95\% CI (log scale); QCPINN and TE-QPINN-FNN reach sub-1\% error on heat but degrade on the broader systems. Row~1: training-loss trajectories. Row~2: periodic-BC violation. Row~3: trainable-scalar accounting (PQC parameters in cool, classical-wrapper parameters in warm), quantifying the classical capacity bundled into the hybrid ans\"atze. \emph{Note:} shock Burgers, the fourth member of the pre-registered PDE ladder, is not shown in this matrix --- coverage is limited to the three smooth/standard systems; the shock-Burgers cells remain queued for the P6 unified-matrix sweep. Sources: \texttt{results/p3\_8\_review/}, \texttt{results/p3\_9\_pde\_matrix/}.}
\label{sup:fig:p3-9-pde-matrix}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_qcpinn_variants.pdf}
\caption{Amendment A17 quantum-parameter step-wise sweep on
\texttt{qcpinn} (Kuramoto, seed 0, SMOKE-budget 50 steps; production
2000 steps via Phase~C on Anvil). Left: training loss vs.\ the
quantum-to-classical parameter ratio $Q / (Q + C)$ at three
amendment-A17 variants ($\approx 24\%$, $45\%$, $87\%$), compared
against the classical PINN baseline (orange dashed). Right: estimated
per-cell wall-clock as a function of quantum capacity. The figure is
labelled SMOKE because the 50-step budget is too short to distinguish
underfitting from family separation; the figure exists to document
that the A17 sweep \emph{is wired and instrumented}, not to support
a Q-helps/Q-hurts conclusion. The production answer will land after
Phase~C re-runs at the uniform 2000-step budget.}
\label{sup:fig:qcpinn-variants}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_loss_history_grid.pdf}
\caption{Training-loss histories for each of the four solver-task
quantum families
(\texttt{chebyshev\_dqc}, \texttt{te\_qpinn\_fnn}, \texttt{te\_qpinn\_qnn},
\texttt{qcpinn}) on each of the four P3.6 ODE systems, with all
three seeds shown as semitransparent overlays and the seed-mean as
a solid line. Vertical axis is the physics-residual loss on log
scale; horizontal axis is the gradient step. The convergence-or-not
diagnostic intended for this figure is the standard ``did training
actually drive the loss down across all 12 (family, system) pairs?''
check that a quantum-PINN benchmark should be able to answer
visually rather than only via the final-loss tables. All 48 (family,
system, seed) cells converge to within $1.5 \times$ of their
seed-mean asymptote across the training budget. Sources:
\texttt{results/p3\_6\_multi\_state/}.}
\label{sup:fig:loss-history-grid}
\end{figure*}
...
The complete amendment record (A1--A22) is at
\texttt{PRE\_REG\_AMENDMENT.md}. Table~\ref{sup:tab:amendments}
summarizes the headline status of each amendment. A1--A13 are
the pre-paper amendments adopted before locking the headline
results; A14 is a post-hoc polish item (Q-Q residual
diagnostics, see \S\ref{sup:qq-diagnostics}); A15--A22 are the
audit-pass amendments adopted after the 5-reviewer peer-review
swarm (2026-05-28), each closing a specific reviewer-flagged
concern.

\begin{table*}[t]
\centering
\caption{Pre-registration amendments (A1--A22). Status column
...
made open without superseding; \textsc{re-run req.} = audit
amendments whose Phase~C re-runs are deferred to the Anvil
allocation (Appendix~A).}
...
\midrule
A15 & equalized training-step budget across ALL solver models (uniform 2000) & DISCLOSED + \textsc{re-run req.} \\
A16 & un-aliased \texttt{strongly\_entangling} from \texttt{data\_reuploading} (PennyLane fallback fix) & DISCLOSED + \textsc{re-run req.} \\
A17 & \texttt{qcpinn} quantum-parameter step-wise sweep along $Q/(Q{+}C)$ ratio (3 variants) & DISCLOSED + 72 new cells \\
A18 & \texttt{brickwall} removed from empirical forecaster sweep (qubit-2 disconnected at $n{=}3,L{=}1$) & DISCLOSED + 9 cells removed \\
A19 & cross-task budget parity: forecaster step budget raised $200 \to 2000$ & DISCLOSED + forecaster re-runs \\
A20 & \texttt{te\_qpinn\_fnn} readout restored \texttt{qml.prod} $\to$ \texttt{qml.sum} (paired-family equivalence) & DISCLOSED + \texttt{fnn} re-runs \\
A21 & \texttt{brickwall} connectivity diagnosis strengthened (linear-chain reduction unless $n_q \geq 4$) & DISCLOSED, no compute \\
